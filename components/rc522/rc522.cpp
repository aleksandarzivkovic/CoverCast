...
// Extended to add MIFARE Ultralight/NTAG page reading and NDEF URI decoding -- see rc522.h.
  ...
  switch (state_) {
    ...
    case STATE_READ_SERIAL_DONE: {
      ...
      if (this->current_uid_ == rfid_uid) {
        // Same tag as last scan; nothing changed, so skip re-reading its NDEF content and don't
        // re-fire on_tag.
        pcd_antenna_off_();
        state_ = STATE_INIT;
        return;
      ...
      this->pending_report_ = report;
      this->start_ndef_read_(std::move(rfid_uid));
      return;
    }
    case STATE_READ_NDEF_CHUNK: {
      // The CRC for this chunk's READ request just finished (buffer_[2..3] now holds it); send it.
      send_len_ = 4;
      pcd_transceive_data_(4);
      state_ = STATE_READ_NDEF_CHUNK_DONE;
      break;
    }
    case STATE_READ_NDEF_CHUNK_DONE: {
      bool got_chunk = status == STATUS_OK && back_length_ >= NDEF_READ_CHUNK_BYTES;
      bool need_more = false;
      if (got_chunk) {
        ndef_raw_.insert(ndef_raw_.end(), buffer_ + send_len_, buffer_ + send_len_ + NDEF_READ_CHUNK_BYTES);

        if (!ndef_header_parsed_) {
          ndef_header_parsed_ = find_mifare_ultralight_ndef_(ndef_raw_, ndef_message_length_, ndef_message_start_index_);
        }
        if (ndef_header_parsed_) {
          size_t needed = 4 + static_cast<size_t>(ndef_message_start_index_) + ndef_message_length_;
          need_more = ndef_raw_.size() < needed;
        }

        size_t chunks_done = ndef_raw_.size() / NDEF_READ_CHUNK_BYTES;
        if (need_more && chunks_done < NDEF_MAX_READ_CHUNKS) {
          ndef_next_page_ += 4;  // 4 pages per 16-byte chunk
          request_ndef_chunk_();
          return;
        }
      }
      state_ = STATE_FINALIZE_TAG;
      break;
    }
    case STATE_FINALIZE_TAG: {
      finalize_tag_();
      break;
    }
    case STATE_DONE: {
...
void RC522::pcd_calculate_crc_(uint8_t *data,  ///< In: Pointer to the data to transfer to the FIFO for CRC calculation.
  ...
  this->crc_result_offset_ = length;
  awaiting_crc_ = true;
  awaiting_comm_ = true;
  awaiting_comm_time_ = millis();
}
...
RC522::StatusCode RC522::await_crc_() {
  ...
  if (n & 0x04) {                               // CRCIRq bit set - calculation done
    ...
    // Transfer the result from the registers to the result buffer, right after the data it was computed over
    buffer_[this->crc_result_offset_] = pcd_read_register(CRC_RESULT_REG_L);
    buffer_[this->crc_result_offset_ + 1] = pcd_read_register(CRC_RESULT_REG_H);
...
void RC522Trigger::process(std::vector<uint8_t> &data, const std::string &url) {
  char uid_buf[format_hex_pretty_size(RC522_MAX_UID_SIZE)];
  this->trigger(format_hex_pretty_to(uid_buf, data.data(), data.size(), '-'), url);
}

// --- NTAG/MIFARE Ultralight page reading + NDEF URI decoding ---
// Reference: esphome/components/pn532/pn532_mifare_ultralight.cpp, adapted from PN532's blocking
// InDataExchange calls to this component's async loop()/awaiting_comm_ state machine, and using
// the raw PICC "MIFARE READ" (0x30) command directly over the RF link instead of PN532's APDU
// wrapper. NDEF message/record parsing itself is reused as-is from esphome/components/nfc.

void RC522::start_ndef_read_(std::vector<uint8_t> &&uid) {
  this->pending_uid_ = std::move(uid);
  this->ndef_raw_.clear();
  this->ndef_header_parsed_ = false;
  this->ndef_message_length_ = 0;
  this->ndef_message_start_index_ = 0;
  // Start at page 3 (Capability Container) like pn532_mifare_ultralight does, so the TLV-locate
  // logic below can be ported byte-for-byte.
  this->ndef_next_page_ = 3;
  this->request_ndef_chunk_();
}

void RC522::request_ndef_chunk_() {
  buffer_[0] = PICC_CMD_MF_READ;
  buffer_[1] = this->ndef_next_page_;
  pcd_calculate_crc_(buffer_, 2);
  state_ = STATE_READ_NDEF_CHUNK;
}

// Ported unchanged (pure byte-offset math, no transport dependency) from
// PN532::find_mifare_ultralight_ndef_. page_3_to_6 is the first 16-byte chunk read (pages 3-6);
// only it is ever checked, since a well-formed tag's NDEF TLV always starts within it.
bool RC522::find_mifare_ultralight_ndef_(const std::vector<uint8_t> &page_3_to_6, uint8_t &message_length,
                                         uint8_t &message_start_index) {
  const uint8_t p4_offset = 4;  // page 4 begins 4 bytes into the vector (page 3 occupies bytes 0..3)
  ...
  if (page_3_to_6[p4_offset + 0] == 0x03) {
    ...
    // A Lock Control TLV preceded the NDEF Message TLV; skip past it.
...
void RC522::finalize_tag_() {
  std::string url;
  if (this->ndef_header_parsed_ && this->ndef_message_length_ > 0) {
    size_t start = 4 + static_cast<size_t>(this->ndef_message_start_index_);
    size_t end = std::min(this->ndef_raw_.size(), start + static_cast<size_t>(this->ndef_message_length_));
    if (end > start) {
      std::vector<uint8_t> message_data(this->ndef_raw_.begin() + start, this->ndef_raw_.begin() + end);
      nfc::NdefMessage message(message_data);
      auto &records = message.get_records();
      if (!records.empty()) {
        url = records[0]->get_payload();
      }
    }
  }
  ...
  std::vector<uint8_t> rfid_uid = std::move(this->pending_uid_);
  this->current_uid_ = rfid_uid;

  for (auto *trigger : this->triggers_ontag_)
    trigger->process(rfid_uid, url);

  if (this->pending_report_) {
    char uid_buf[format_hex_pretty_size(RC522_MAX_UID_SIZE)];
    ESP_LOGD(TAG, "Found new tag '%s' url='%s'", format_hex_pretty_to(uid_buf, rfid_uid.data(), rfid_uid.size(), '-'),
             url.c_str());
  }
}
