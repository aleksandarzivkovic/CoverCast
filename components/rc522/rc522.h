...
// Forked from the ESPHome 2026.8.2 built-in `rc522` component to add
// MIFARE Ultralight / NTAG plain-page reading and NDEF URI decoding
// (via the shared esphome/components/nfc library, as used by pn532).
// See CoverCast repo history for the diff against upstream.
...
class RC522 : public PollingComponent {
 ...
 protected:
  ...
  enum State {
    ...
    STATE_READ_NDEF_CHUNK,       // CRC for a page-read request has just finished; kick off the transceive
    STATE_READ_NDEF_CHUNK_DONE,  // page-read transceive has just finished; append data, decide if more is needed
    STATE_FINALIZE_TAG,          // NDEF reading done (or skipped); decode URL, turn antenna off, fire triggers
    STATE_DONE,
  } state_{STATE_NONE};
  ...
  bool awaiting_comm_;
  // True if the pending background operation is a CRC calculation (await_crc_) rather than a
  // transceive (await_transceive_). Generalized from the original code's hardcoded state check
  // so any state can kick off a CRC calculation, not just STATE_SELECT_SERIAL.
  bool awaiting_crc_{false};
  uint32_t awaiting_comm_time_;
  StatusCode await_transceive_();
  StatusCode await_crc_();

  // Buffer sized for a MIFARE READ response: 4-byte command (cmd+page+CRC16) sent, up to 16 data
  // bytes + 2 CRC bytes received. The original 9-byte buffer only ever needed to fit anticollision
  // and SELECT frames.
  static constexpr uint8_t RC522_BUFFER_SIZE = 24;
  uint8_t buffer_[RC522_BUFFER_SIZE];  ///< buffer for communication, the first bits [0..back_idx-1] are for tx ,
  ...
  uint8_t rx_align_;
  uint8_t *valid_bits_;
  uint8_t crc_result_offset_{0};  // where in buffer_ pcd_calculate_crc_'s result should be written
  ...
  // A MIFARE READ always returns 4 pages (16 bytes) regardless of how many pages you actually need.
  static constexpr uint8_t NDEF_READ_CHUNK_BYTES = 16;
  // Hard cap on chunk reads per tag (10 * 16 = 160 bytes), comfortably covers NTAG213/215 user memory.
  static constexpr uint8_t NDEF_MAX_READ_CHUNKS = 10;
  std::vector<uint8_t> pending_uid_;  // UID captured before starting the NDEF read phase
  bool pending_report_ = true;        // whether to log "Found new tag" once finalized (see original `report`)
  std::vector<uint8_t> ndef_raw_;     // raw bytes accumulated starting at page 3
  uint8_t ndef_next_page_ = 0;
  bool ndef_header_parsed_ = false;
  uint8_t ndef_message_length_ = 0;
  uint8_t ndef_message_start_index_ = 0;

  static bool find_mifare_ultralight_ndef_(const std::vector<uint8_t> &page_3_to_6, uint8_t &message_length,
...
// Second argument is the decoded NDEF URI payload, or "" if the tag had none / isn't NDEF-formatted.
// on_tag_removed always passes "" for it.
class RC522Trigger final : public Trigger<std::string, std::string> {
 public:
  void process(std::vector<uint8_t> &data, const std::string &url);
};
